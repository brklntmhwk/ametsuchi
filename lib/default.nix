...
{ inputs, lib, ... }:
{
	mkEmacsConfig =
		{
			pkgs,
			emacsPackage ? (import inputs.emacs-overlay).packages.${pkgs.system}.emacs-pgtk,
			initFile,
		}:
let
	twistArgs = {
...
in
	(inputs.twist.lib.makeEnv (twistArgs // {
		...
		(lib.composeExtensions inputs.twist-overrides.overlays.twistScope
			(_final: prev: {
				elispPackages = prev.elispPackages.overrideScope (
					import ../twist/overrides.nix { inherit pkgs; }
				);
			}));
}
