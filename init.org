...
(use-package modus-themes
  ...
  :config
  (defvar brk/noctalia-theme-file (expand-file-name "noctalia-theme.el" user-emacs-directory))
  (if (file-exists-p brk/noctalia-theme-file)
      (progn
        (load brk/noctalia-theme-file)
...
(use-package startup
  ...
  ;; `dashboard-setup-startup-hook' takes care of this too.
  ;; (initial-buffer-choice
  ;;  (lambda () (get-buffer-create "*dashboard*")))
  (initial-scratch-message nil)
  (initial-major-mode 'fundamental-mode)
  :config
  (defun brk/server-edit-prevent-dashboard-ad (orig-fn &rest args)
    "Temporarily bind `initial-buffer-choice' to nil during `server-edit'
to prevent Dashboard from hijacking the frame after e.g., a Magit commit."
    (let ((initial-buffer-choice nil))
      (apply orig-fn args)))
  (advice-add 'server-edit :around #'brk/server-edit-prevent-dashboard-ad))
#+end_src
*** Development
:PROPERTIES:
...
(use-package magit
  ...
  (defun brk/magit-restore-status-sync-ad (orig-fn &rest args)
    "Synchronously restore `magit-status' after `with-editor-finish'.
This prevents aggressive fallback to the Dashboard buffer while waiting for you to complete
git signing (e.g., a YubiKey touch).
Restricted to run only if you are finishing a git-commit buffer to prevent side-effects."
    (let ((is-git-commit (derived-mode-p 'git-commit-mode))
          (repo-dir default-directory))
      (apply orig-fn args)
      (when is-git-commit
        (magit-status repo-dir))))
  (advice-add 'with-editor-finish :around #'brk/magit-restore-status-sync-ad))
#+end_src
***** magit-delta
#+begin_quote
