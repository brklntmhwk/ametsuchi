...
(use-package which-func
  ...
  ;; This avoids crashing strict servers like nil (i.e., NIx Language server) with "File not loaded"
  ;; errors, leaving the update safely to the default idle timer instead.
...
(use-package eglot
  ...
  (defun brk/eglot-xref-identifier-ad (orig-fn backend)
    "Return the real identifier for Eglot fallback instead of the dummy string."
    (if (and (consp backend)
             (eq (car backend) 'eglot)
             (cdr backend))
        (let ((real-id (xref-backend-identifier-at-point (cdr backend))))
          (or real-id (funcall orig-fn backend)))
      (funcall orig-fn backend)))
  (advice-add 'xref-backend-identifier-at-point :around #'brk/eglot-xref-identifier-ad))
#+end_src
***** eglot-booster
#+begin_quote
