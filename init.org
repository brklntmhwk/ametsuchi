...
(use-package eglot
  ...
  (defun brk/fix-eglot-fallback-ad (orig-fn backend identifier)
    "Fix Eglot's bug where it passes its dummy identifier to fallback backends."
    (if (and (stringp identifier)
             (string= identifier "LSP identifier at point")
             (not (eq backend 'eglot)))
        (let ((real-id (xref-backend-identifier-at-point backend)))
          (if real-id
              (funcall orig-fn backend real-id)
            nil))
      (funcall orig-fn backend identifier)))
  (advice-add 'xref-backend-definitions :around #'brk/fix-eglot-fallback-ad))
#+end_src
***** eglot-booster
#+begin_quote
