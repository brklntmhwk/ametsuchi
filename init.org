...
Prevents fallback buffers (like Dashboard) from lingering in the commit window
without destroying the window layout and without screen flickering."
    (let ((commit-window (selected-window))
          (is-git-commit (bound-and-true-p git-commit-mode))
          (repo-dir default-directory))
      (let ((inhibit-redisplay t))
        (apply orig-fn args)
        (when (and is-git-commit
                   (window-live-p commit-window)
                   (string= (buffer-name (window-buffer commit-window)) "*dashboard*"))
          (when-let ((status-buf (magit-status-setup-buffer repo-dir)))
            (set-window-buffer commit-window status-buf))))))
