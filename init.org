...
(use-package org-capture
  ...
  (defun my/org-capture-find-latest-datetree-entry ()
...
If not, create it using `org-reverse-datetree-goto-date-in-file' before placing point."
    (let* ((journal-dir (file-name-as-directory (concat user-documents-dir "journal/")))
      ...
      (unless (file-exists-p latest-file)
        (with-temp-buffer
          (insert (format "* %s\n"
                          (format-time-string org-reverse-datetree-year-format)))
          (write-file latest-file))
  ...
  (setq org-capture-templates
        `(("a" "Anki Basic" entry
          ...
          ("d" "Draft" entry
           ...
           :prepend t)
          ("j" "Journal" item
           (function my/org-capture-find-latest-datetree-entry)
           "%U %?")
          ("p" "Project" entry
           (file "")
           ,(concat
