...
(use-package consult
  ...
  :config
  (require 'brk-consult)
  (add-to-list 'consult-buffer-sources 'brk-consult--source-activities-buffer))
#+end_src
***** consult-dir
#+begin_quote
...
NOTE: Key bindings related to scrolling are defined in the [[#window-management][window]] configuration with some custom commands.
