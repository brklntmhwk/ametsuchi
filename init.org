...
| Package manager     | N/A           | package.el, straight.el, elpaca, twist.el etc.  |
| Configuration Macro | use-package   | leaf.el, setup.el, etc.                         |
| Nix framework | N/A           | flake-parts, flake-utils, snowfall, denix, etc. |
...
*** As a module
:PROPERTIES:
:CUSTOM_ID: as-a-module
:END:
Ametsuchi provides modules for home-manager and nix-maid. If you use Nix Flakes, the setup is as simple as:

#+begin_src nix
{
  inputs = {
    ametsuchi = {
      url = "github:brklntmhwk/ametsuchi";
  ...
  outputs =
    { self, nixpkgs, ametsuchi, home-manager, nix-maid }: { 
      nixosConfigurations.yourhostname = nixpkgs.lib.nixosSystem {
        system = "x86_64-linux";
        modules = [
          ./configuration.nix
          # Nix-maid Module
          nix-maid.nixosModules.default # Ensure to add the nix-maid module for NixOS
          ametsuchi.maidModules.ametsuchi

          # Home Manager Module
          home-manager.nixosModules.home-manager # Ensure to add the home-manager module for NixOS
          {
            home-manager.users.youruser = { pkgs, ... }: {
              imports = [
                ametsuchi.homeManagerModules.ametsuchi
              ];
            };
...
Now you can start using the module options ~programs.ametsuchi~ anywhere in your module system.
...
Add this as early as possible to the init file.
...
#+end_quote
#+begin_src emacs-lisp
;; Disable Fcitx inputs and switches when using Emacs.
...
(use-package mozc
  ...
  :demand t
  :config
  (defun brk/mozc-candidate-next ()
    (interactive)
    (mozc-handle-event 'down))
  (defun brk/mozc-candidate-prev ()
    (interactive)
    (mozc-handle-event 'up))
  (keymap-set mozc-mode-map "C-n" #'brk/mozc-candidate-next)
  (keymap-set mozc-mode-map "C-p" #'brk/mozc-candidate-prev))
#+end_src
***** mozc-popup
#+begin_quote
Mozc with popup
...
(use-package popper
  ...
  :config
  ;; Based on:
  ;; https://github.com/seagle0128/.emacs.d/blob/7b88e0852a0e0d85d3e6ceb6041addc9bf9193b8/lisp/init-window.el
  (defun brk/popper-close-via-keyboard-quit-ad (&rest _)
    "Dismiss the latest popper window via `keyboard-quit' (usually `C-g').
This does not close the window if a region is active, or if the user is currently
inside a terminal/shell popup where `C-g' is needed for interrupts."
    (when (and (not (region-active-p))
               popper-open-popup-alist)
      (let* ((popup (car popper-open-popup-alist))
             (window (car popup))
             (buffer (cdr popup)))
        (when (and (window-live-p window)
                   (buffer-live-p buffer)
                   ;; Don't close if the cursor is inside a terminal/shell popup.
                   ;; This allows `C-g' to safely send interrupts (SIGINT) to the process.
                   ;; If the cursor is in the main window, `C-g' will dismiss the terminal.
                   (not (and (eq (selected-window) window)
  ...
  (advice-add 'keyboard-quit :before #'brk/popper-close-via-keyboard-quit-ad))
#+end_src
**** tab-bar
#+begin_quote
...
(use-package tab-bar
  ...
  :config
  ;; Call `find-file' safely in an idle state after executing `tab-new'.
  (defun brk/tab-new-find-file-ad (&rest _)
    (run-with-timer 0 nil (lambda () (call-interactively #'find-file))))
  (advice-add 'tab-new :after #'brk/tab-new-find-file-ad))
#+end_src
**** window
#+begin_quote
...
(setq-default buffer-file-coding-system 'utf-8 ; `undecided-unix' by default
              ...
              ;; Make this a bit longer so the diff indicators have enough space to appear.
              left-fringe-width 16 ; nil by default
...
(use-package auto-dark
  :ensure t
  ;; Don't enable this on WSL as it's not compatible with any theme detection mechanism
  ;; auto-dark uses (e.g., xdg-desktop-portal).
  :if (not (getenv "WSL_DISTRO_NAME")) ; e.g., "NixOS"
  :after modus-themes
  :custom
  (auto-dark-themes '((modus-vivendi-tinted) (modus-operandi-tinted)))
  :hook
  (after-init-hook . auto-dark-mode)
  ;; Re-evaluate those custom faces defined in `modus-themes-with-colors' every time
  ;; auto-dark switches the theme (dark/light).
  (auto-dark-dark-mode-hook . brk/modus-themes-custom-face)
  (auto-dark-light-mode-hook . brk/modus-themes-custom-face))
#+end_src
**** fontaine
#+begin_quote
...
(use-package fontaine
  ...
  (fontaine-presets
   '((regular
      :default-family "Moralerspace Neon HW"
      :default-height 130
      :fixed-pitch-family "Moralerspace Neon HW"
...
(use-package modus-themes
  ...
  (defun brk/modus-themes-custom-face ()
    (modus-themes-with-colors
      (custom-set-faces
       ;; ace-window
       `(aw-leading-char-face ((,c :height 2.0 :foreground ,slate)))
       `(aw-minibuffer-leading-char-face ((,c :height 1.1 :foreground ,slate)))
       ;; blamer
       `(blamer-face ((,c :height 1.0 :background ,bg-hl-line :foreground ,fg-lavender :italic t)))
       ;; dashboard
       `(dashboard-text-banner ((,c :family "monospace" :foreground ,blue-warmer)))
       ;; dired-filter
...
#+begin_quote
[!NOTE]
Ensure to install Nerd Fonts individually.
#+end_quote
#+begin_src emacs-lisp
...
(use-package nerd-icons-completion
  :ensure t
  :after marginalia
  :hook (marginalia-mode-hook . nerd-icons-completion-mode)
  :config
  ;; Call the dedicated setup function directly not to miss the hook timing.
...
(use-package nerd-icons-dired
  ...
  :config
  (defun brk/nerd-icons-dired-fix-hook-order ()
    "Ensure `nerd-icons-dired--refresh' runs last in `dired-after-readin-hook'.
This prevents icons from being wiped out by other packages that reconstruct the Dired
buffer (e.g, `dired-filter', `dired-collapse', etc)."
    (when nerd-icons-dired-mode
      (remove-hook 'dired-after-readin-hook #'nerd-icons-dired--refresh t)
      ;; Append `nerd-icons-dired--refresh' to `dired-after-readin-hook'.
      (add-hook 'dired-after-readin-hook #'nerd-icons-dired--refresh t t)))
  (add-hook 'nerd-icons-dired-mode-hook #'brk/nerd-icons-dired-fix-hook-order))
#+end_src
**** olivetti
#+begin_quote
...
(use-package which-func
  ...
  :config
  ;; Prevent a race condition where `which-func-ff-hook' synchronously requests document
  ;; symbols right after a file is opened, often before the LSP server has fully processed
  ;; the "textDocument/didOpen" notification.
  ;; Since `which-function-mode' aggressively re-adds itself to `find-file-hook' upon
  ;; activation, completely neutralize the function via advice.
  (advice-add 'which-func-ff-hook :override #'ignore))
#+end_src
*** Completion
:PROPERTIES:
...
(use-package minibuffer
  ...
  :config
  (defun brk/minibuffer-isolate-kill-ring-ad (orig-fn &rest args)
    "Isolate `kill-ring' and clipboard in the minibuffer."
    (if (minibufferp)
        ;; Shadow the kill-ring variables locally and prevent OS clipboard syncing,
        ;; allowing the killed text to vanish without a trace.
        (let ((kill-ring kill-ring)
              (kill-ring-yank-pointer kill-ring-yank-pointer)
              (interprogram-cut-function nil))
          (apply orig-fn args))
      ;; Fall back to the default behaviour outside the minibuffer.
      (apply orig-fn args)))
  ;; Wrap the foundational kill-region command to intercept all kill operations.
  (advice-add 'kill-region :around #'brk/minibuffer-isolate-kill-ring-ad))
#+end_src
**** orderless
#+begin_quote
...
(use-package vertico
  ...
  :bind
  (:map vertico-map
        ;; Align this keybinding with the existing `vertico-directory-enter' command.
        ("C-<return>" . brk/vertico-exit-other-tab))
  :config
  (defvar brk/vertico-exit-other-tab-flag nil
    "Flag to force the next buffer to open in a new tab.")

  ;; Intercept `display-buffer' at the lowest level of Emacs's window management.
  (defun brk/display-buffer-other-tab-ad (orig-fn buffer-or-name &optional action frame)
    "Open the buffer in a new tab if `brk/vertico-exit-other-tab-flag' is set."
    (if brk/vertico-exit-other-tab-flag
        (progn
          ;; Consume the flag immediately to keep the state ephemeral and localized.
          (setq brk/vertico-exit-other-tab-flag nil)
          ;; Temporarily enforce the overriding rule to open in a new tab.
          (let ((display-buffer-overriding-action '((display-buffer-in-new-tab))))
            (funcall orig-fn buffer-or-name action frame)))
      (funcall orig-fn buffer-or-name action frame)))
  (advice-add 'display-buffer :around #'brk/display-buffer-other-tab-ad)

  (defun brk/vertico-exit-other-tab ()
    "Exit completing-read and perform the default action in a new tab."
    (interactive)
    (setq brk/vertico-exit-other-tab-flag t)
    ;; As a safety net, completely purge the flag when Emac turns idle, ensuring
    ;; no stray state is left behind in case of aborted actions.
    (run-with-idle-timer 0 nil (lambda () (setq brk/vertico-exit-other-tab-flag nil)))
    (vertico-exit)))
#+end_src
***** vertico-directory
#+begin_quote
...
(use-package vertico-directory
  ...
  (:map vertico-map
        ...
        ;; Delete the previous directory path component cleanly instead of killing it.
...
(use-package vertico-multiform
  ...
  (vertico-multiform-commands
   '((consult-buffer (:not posframe))
     ...
     (embark-dwim (:not posframe))
     (embark-become (:not posframe))
     (t posframe)))
  :hook
...
(use-package avy
  ...
  (setf (alist-get ?j avy-dispatch-alist) #'brk-avy-action-xref-definitions)
  (setf (alist-get ?. avy-dispatch-alist) #'brk-avy-action-embark)
  (setf (alist-get ?  avy-dispatch-alist) #'brk-avy-action-mark-to-char))
#+end_src
**** deadgrep
#+begin_quote
...
(use-package newcomment
  :bind
  ([remap comment-dwim] . brk/comment-real-dwim)
  :config
  (defun brk/comment-real-dwim ()
    "True DWIM comment command.
If a region is active, comment or uncomment the line(s) spanned by the region, otherwise
the current line."
    (interactive)
    (let ((beg (if (use-region-p)
          ...
          (end (if (use-region-p)
                   (save-excursion
                     ...
                     ;; Exclude that new line if the region end is at the beginning of
                     ;; the next line.
                     (if (and (bolp) (not (eq (region-beginning) (region-end))))
                         (point)
                       (line-end-position)))
                 (line-end-position))))
      (save-excursion
...
(use-package puni
  ...
  (mapc (lambda (k) (keymap-unset puni-mode-map k))
        '("C-M-a" "C-M-e" "C-M-f" "C-M-b" "C-d" "M-d" "M-DEL")))
#+end_src
**** replace
#+begin_quote
...
(use-package sontaku
  :ensure t
  :hook
  (after-init-hook . sontaku-mode)
  ((markdown-mode-hook
    org-mode-hook)
   . sontaku-apply-word-boundaries))
#+end_src
**** string-inflection
#+begin_quote
...
(use-package display-line-numbers
  ...
  (defun brk/display-line-numbers-fixed-width ()
    (when (< display-line-numbers-width 4)
...
(use-package goggles
  ...
  (defvar-local brk/goggles-mode-saved-state nil)
  ;; To avoid the repetitive pulses every time the point moves across the candidates,
  ;; disable goggles when mozc is enabled.
  (with-eval-after-load 'mozc
    (add-hook 'mozc-mode-hook
              (lambda ()
                (if mozc-mode
                    (progn
                      (setq brk/goggles-mode-saved-state goggles-mode)
                      (goggles-mode -1))
                  (when brk/goggles-mode-saved-state
                    (goggles-mode 1)))))))
#+end_src
...
(use-package dashboard
  ...
  (setopt dashboard-banner-ascii brk-dashboard-banner-ascii-text)
  ...
  ;; Add "*dashboard*" buffer to the blacklist of target buffers to automatically
  ;; to switch to as a fallback when a buffer is killed.
  (setq switch-to-prev-buffer-skip
        (lambda (_win buf _bury-or-kill)
          (string= (buffer-name buf) "*dashboard*"))))
#+end_src
**** dired
#+begin_quote
...
(use-package envrc
  ...
  :config
  (defun brk/envrc-propagate-org-babel-ad (orig-fn &rest args)
    "Propagate the buffer-local envrc environment to org-babel.
org-babel uses temporary buffers to execute commands, which normally
do not inherit the `envrc-mode' environment. This wrapper explicitly
injects the environment into the execution context."
    (if (bound-and-true-p envrc-mode)
        (envrc-propagate-environment
         (apply orig-fn args))
      (apply orig-fn args)))

  (advice-add 'org-babel-execute-src-block :around #'brk/envrc-propagate-org-babel-ad))
#+end_src
**** files
#+begin_quote
...
(use-package project
  :custom
  ;; Emacs 29+: Configure this to get `project' to recognize the subproject's root files
  ;; while keeping the VCS's faster file search.
...
(use-package eglot
  ...
  (defun brk/eglot-xref-read-id-ad (orig-fn backend)
    "Replace Eglot's dummy string with the real symbol at point for fallback backends."
    (let ((id (funcall orig-fn backend)))
      (if (string= id "LSP identifier at point")
          (or (thing-at-point 'symbol t) id)
        id)))
  (advice-add 'xref-backend-identifier-at-point :around #'brk/eglot-xref-read-id-ad))
#+end_src
***** eglot-booster
#+begin_quote
...
(use-package php-ts-mode
  :mode "\\.php\\'"
  :hook (php-ts-mode-hook . eglot-ensure))
#+end_src
***** rust-ts-mode
#+begin_quote
...
(use-package magit
  ...
  :config
  ;; NOTE: We don't rely on this async hook anymore as it fails to handle some edge cases
  ;; properly (e.g., sign a commit with a YubiKey).
  ...
  (defun brk/with-editor-prevent-dashboard-ad (orig-fn &rest args)
    "Synchronously restore `magit-status' buffer after with-editor completion.
  This prevents fallback buffers (e.g., Dashboard) from lingering in the commit window
  while waiting for synchronous tasks like GPG signing."
    (let ((is-git-commit (bound-and-true-p git-commit-mode))
          (repo-dir (magit-toplevel)))
      (let ((inhibit-redisplay t))
        (apply orig-fn args)
        (when (and is-git-commit repo-dir)
          (magit-status repo-dir)))))

  (advice-add 'with-editor-finish :around #'brk/with-editor-prevent-dashboard-ad)
  (advice-add 'with-editor-cancel :around #'brk/with-editor-prevent-dashboard-ad)

  (transient-append-suffix 'magit-commit "-S"
    '("-N" "Sign without gpg" "--no-gpg-sign")))
#+end_src
***** magit-delta
#+begin_quote
...
(use-package browse-url
  ...
  :bind
  (:map system-meta-map
        ("b" . brk-browse-url-choose-browser))
  :init
  (require 'brk-browse-url)
  (brk-browse-url-initialize-browser))
#+end_src
**** eww
#+begin_quote
Emacs Web Wowser
...
(use-package eat
  ...
  :bind
  ((:map project-prefix-map
         ;; Run `eat' in the project root if available.
         ("t" . eat-project)))
  :config
...
[2025-10-17 Fri] Migrated my diary system from Org-journal to Org-reverse-datatree, for it can handle date entries in reverse chronological order. That said, it still needs to ally with Org-capture to fully compete against Org-journal though.
