...
(defcustom my/fontset-config-alist
  ...
  :type '(alist
          :key-type ((choice
                      (list (const :charsets) (repeat symbol))
                      (list (const :ranges) (repeat (cons integer integer)))
                      (list (const :scripts) (repeat symbol))))
          :value-type (plist
                       :options
                       (((choice
...
CONFIG-LIST defaults to `my/fontset-config-alist'.
SKIP-CHECK bypasses font availability checking."
  (let ((available-fonts (font-family-list))
    ...
    (cl-loop for (target . plist) in fontsets
    ...
    (pcase characters
      (`(:charsets ,charsets)
       (dolist (charset charsets)
         (set-fontset-font fontset charset font-spec nil add-method)))
      (`(:ranges ,ranges)
       (cl-loop for (from . to) in ranges
                do (set-fontset-font fontset (cons from to) font-spec nil add-method)))
      (`(:scripts ,scripts)
       (dolist (script scripts)
         (set-fontset-font fontset script font-spec nil add-method))))))
...
(use-package modus-themes
  ...
  (modus-themes-headings
   '((1 . (variable-pitch bold 1.6))
     (2 . (variable-pitch rainbow bold 1.5))
     (3 . (variable-pitch rainbow semibold 1.4))
     (4 . (variable-pitch rainbow semibold 1.3))
     (5 . (variable-pitch rainbow medium 1.2))
     (t . (1.1))))
...
(use-package magit
  ...
  ;; (magit-clone-default-directory "~/Documents/projects")
  :bind
  ((:map ctl-x-map
...
(use-package org-agenda
  ...
  (org-agenda-files '(user-documents-dir))
  ...
  :config
  ;; Unset `org-agenda-goto' because it's confusing.
  (mapc (lambda (k) (keymap-unset org-agenda-mode-map k))
        '("TAB" "<tab>"))
  ;; Then override `delete-other-windows' because I don't use it.
...
(use-package org-superstar
  ...
  :after org
  :custom
  ;; TODO: Consider using chess pieces as bullets
