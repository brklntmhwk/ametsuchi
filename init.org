...
#+end_src
**** Suppression of lexical binding warnings
This prevents legacy upstream files (e.g., mozc.el) from triggering missing lexical-binding warnings introduced in Emacs 30+.

#+begin_src emacs-lisp
(defun brk/suppress-lexical-binding-warning-ad (orig-fn type message &rest args)
  "Silence specific files-category warnings regarding missing lexical-binding cookies."
  (if (and (eq type 'files)
           (stringp message)
           ;; Periods surrounding "lexical-binding" handle the difference in quotation
           ;; marks (e.g., ` ' and ' ') used.
...
(use-package eglot
  ...
  (defun brk/eglot-xref-fallback-ad (orig-fn backend identifier)
    "Force fallback to `dumb-jump' when `eglot-xref-backend' returns nil."
    (let ((action (car args))
          (res (apply orig-fn args)))
      ;; If Eglot finds no definitions or references either...
      (if (and (memq action '(definitions references)) (null res))
          (let ((dj-backend (and (fboundp 'dumb-jump-xref-activate)
                                 (dumb-jump-xref-activate))))
            (if dj-backend
                ;; Ignore the dummy string "LSP identifier at point", passing the
                ;; actual symbol at point to `dumb-jump' and looking it up again.
                (let ((identifier (xref-backend-identifier-at-point dj-backend)))
                  (xref-backend-definitions dj-backend identifier))
                res))
        res)))
  (advice-add 'eglot-xref-backend :around #'brk/eglot-xref-fallback-ad))
#+end_src
***** eglot-booster
#+begin_quote
