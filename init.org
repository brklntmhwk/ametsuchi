...
(use-package org-agenda
  ...
  ;; Stick to DRY
  (defun my/org-agenda-common-agenda-sections ()
  ...
  (setq org-agenda-custom-commands
        `(("@" "Private Agenda"
           ,(my/org-agenda-common-agenda-sections)
           ((org-agenda-files
             (seq-filter
              (lambda (file)
                (and
          ...
          ("w" "Workplace Agenda"
           ...
           ((org-agenda-files
            ...
            (org-agenda-block-separator nil)))
          ("x" "The Miscellaneous Task Views"
...
(use-package org-capture
  ...
  :config
  (require 'org-journal)
  (defun my/org-capture-find-latest-journal-entry ()
...
If not, create it using `org-journal-new-entry' before placing point."
    (let* ((journal-dir (file-name-as-directory org-journal-dir))
           (org-files (directory-files journal-dir t "^[0-9]+\\.org$"))
           (latest-file (car (sort org-files
                                   (lambda (a b)
                                     (> (string-to-number (file-name-base a))
                                        (string-to-number (file-name-base b))))))))
      (if (not latest-file)
          (user-error "No yearly journal files found in %s" journal-dir)
        ...
        (widen)
        (let* ((today-string (format-time-string org-journal-date-format))
               (today-heading-re
                (format "^%s%s"
                        (regexp-quote org-journal-date-prefix)
                        (regexp-quote today-string))))
          (goto-char (point-min))
          (if (re-search-forward today-heading-re nil t)
              (progn
                (org-end-of-subtree)
                (unless (bolp) (insert "\n")))
            (goto-char (point-max))
  ...
  (setq org-capture-templates
        `(("a" "Anki Basic" entry
          ...
          ("c" "Anki Cloze" entry
           ...
           :prepend t)
          ("d" "Draft" plain
           (file+headline "" "Drafts")
           "%U %?"
           :empty-lines 1
           :prepend t)
          ("j" "Journal" item
           (function my/org-capture-find-latest-journal-entry)
           "%U %?")
          ("p" "Project" entry
           (file+headline "" "Projects")
           ,(concat
             "* PENDING %? :@PROJECT:\n"
             ":LOGBOOK:\n"
             "- Created at      %U\n"
